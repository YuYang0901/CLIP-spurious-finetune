...
class ImageNetSubset(Dataset):
    def __init__(self, dataset, class_index, transform):
...
def compute_mi(config, vocab, objects, class_indices):
    ...
    for class_idx in class_indices:
        ...
        try:
            detect = np.load(os.path.join(config.save_dir, f'detect_{config.split}_class{class_idx}_{config.model}_threshold{config.score_threshold}.npy'))
            if np.shape(detect)[1] > 10000:
                continue
            correct = np.load(os.path.join(config.save_dir, f'correct_{config.split}_class{class_idx}_{config.model}.npy'))
        except:
            continue
