...
from wilds.common.metrics.all_metrics import (
    binary_logits_to_pred, multiclass_logits_to_pred,
    pseudolabel_binary_logits, pseudolabel_detection,
    pseudolabel_detection_discard_empty, pseudolabel_identity,
    pseudolabel_multiclass_logits)

from utils import (collate_list, detach_and_clone,
                   get_model_prefix, get_pred_prefix, save_model, save_pred)
import wandb
...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train):
    ...
    for labeled_batch in batches:
        ...
        if config.algorithm == 'Multimodal':
            for field in batch_results['spur_true']:
                if field not in epoch_spur_true:
                    epoch_spur_true[field] = []
                    epoch_spur_pred[field] = []
                epoch_spur_true[field].append(detach_and_clone(batch_results['spur_true'][field]))
                spur_pred = detach_and_clone(batch_results['spur_pred'][field])
                if config.process_outputs_function is not None:
                    spur_pred = process_outputs_functions[config.process_outputs_function](spur_pred)
                epoch_spur_pred[field].append(spur_pred)
    ...
    for field in epoch_spur_true:
        epoch_spur_pred[field] = collate_list(epoch_spur_pred[field])
        epoch_spur_true[field] = collate_list(epoch_spur_true[field])
        results_, _ = dataset['dataset'].eval(
            epoch_spur_pred[field],
            epoch_spur_true[field],
            epoch_metadata)
        keys = list(results_.keys())
        for key in keys:
            results[f'{field}_{key}'] = results_.pop(key)
...
def train(algorithm, datasets, general_logger, config, epoch_offset, best_val_metric):
    ...
    Train loop that, each epoch:
        - Steps an algorithm on the datasets['train'] split
    ...
    for epoch in range(epoch_offset, config.n_epochs):
        ...
        for split in additional_splits:
            eval_results, y_pred = run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)
            save_pred_if_needed(y_pred, datasets[split], epoch, config, is_best)

            if is_best and config.use_wandb:
                results = {}
                for key in eval_results:
                    new_key = f'best/{key}'
                    results[new_key] = eval_results[key]
                wandb.log(results)
...
def get_features(algorithm, dataloader):
    all_features = []
    all_labels = []
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader):
            try:
                features = algorithm.model.model.encode_image(images.to(algorithm.device))
            except:
                features = algorithm.model.module.model.encode_image(images.to(algorithm.device))
